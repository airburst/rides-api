ALTER TABLE "public"."accounts" ENABLE ROW LEVEL SECURITY;--> statement-breakpoint
ALTER TABLE "public"."archived_rides" ENABLE ROW LEVEL SECURITY;--> statement-breakpoint
ALTER TABLE "public"."archived_users_on_rides" ENABLE ROW LEVEL SECURITY;--> statement-breakpoint
ALTER TABLE "public"."club_api_keys" ENABLE ROW LEVEL SECURITY;--> statement-breakpoint
ALTER TABLE "public"."clubs" ENABLE ROW LEVEL SECURITY;--> statement-breakpoint
ALTER TABLE "public"."memberships" ENABLE ROW LEVEL SECURITY;--> statement-breakpoint
ALTER TABLE "public"."repeating_rides" ENABLE ROW LEVEL SECURITY;--> statement-breakpoint
ALTER TABLE "public"."rides" ENABLE ROW LEVEL SECURITY;--> statement-breakpoint
ALTER TABLE "public"."sessions" ENABLE ROW LEVEL SECURITY;--> statement-breakpoint
ALTER TABLE "public"."user_clubs" ENABLE ROW LEVEL SECURITY;--> statement-breakpoint
ALTER TABLE "public"."users" ENABLE ROW LEVEL SECURITY;--> statement-breakpoint
ALTER TABLE "public"."users_on_rides" ENABLE ROW LEVEL SECURITY;--> statement-breakpoint
ALTER TABLE "public"."verification_tokens" ENABLE ROW LEVEL SECURITY;
